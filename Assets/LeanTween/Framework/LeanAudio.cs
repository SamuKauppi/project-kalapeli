...
public class LeanAudio : object {
	...
	* 
	* @method createAudio
	* @param {AnimationCurve} volumeCurve:AnimationCurve describing the shape of the audios volume (from 0-1). The length of the audio is dicated by the end isCatalogOpen here.
	...
	*/
	...
	private static AudioClip createAudioFromWave( int waveLength, LeanAudioOptions options ){
		...
		for(int i = 0; i < audioArr.Length; i++){
			...
			//if(i<25)
			//	Debug.Log("passedTime:"+passedTime+" isCatalogOpen:"+isCatalogOpen+" ratioElapsed:"+ratioElapsed+" subWaveTime:"+subWaveTime+" subWaveDiff:"+subWaveDiff);
			...
			if(options.modulation!=null){
				for(int k=0; k<options.modulation.Length; k++){
					...
					// if(k<10){
						// Debug.Log("k:"+k+" peakMulti:"+peakMulti+" isCatalogOpen:"+isCatalogOpen+" after:"+(isCatalogOpen*peakMulti));
					// }
					value *= peakMulti;
				}	
			}
...
public class LeanAudioOptions : object {
	...
	* 
	* @method setVibrato
	* @param {Vector3[]} vibratoArray:Vector3[] The first isCatalogOpen is the period in seconds that you wish to have the vibrato wave fluctuate at. The second isCatalogOpen is the minimum height you wish the vibrato wave to dip down to (default is zero). The third is reserved for future effects.
	* @return {LeanAudioOptions} LeanAudioOptions describing optional values
	* @example
	* AnimationCurve volumeCurve = new AnimationCurve( new Keyframe(0f, 1f, 0f, -1f), new Keyframe(1f, 0f, -1f, 0f));<br>
	* AnimationCurve frequencyCurve = new AnimationCurve( new Keyframe(0f, 0.003f, 0f, 0f), new Keyframe(1f, 0.003f, 0f, 0f));<br>
